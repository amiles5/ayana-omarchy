...
Full effective keybinding list for `ayana`'s Omarchy install — Omarchy's own stock bindings
(`/usr/share/omarchy/default/hypr/bindings.lua`) plus this machine's overrides in
[`.config/hypr/bindings.lua`](.config/hypr/bindings.lua) (currently: `SUPER+SHIFT+D` repurposed
from the default Docker TUI bind to launch Moneydance).

Generated by running the live source of truth rather than hand-transcribed from either bindings
file, so it reflects both stock and overridden binds as Hyprland actually sees them:

```
omarchy menu keybindings --print
```

Regenerate the same way if bindings change — this file is a snapshot (2026-09-21), not
auto-synced.

Two keys appear twice below (`ALT + TAB`, `SHIFT ALT + TAB`) — Omarchy binds them to different
actions depending on context (e.g. window grouping), both live simultaneously.
